...
'------------------------------------------------------------
function fetch_HTML_JSON(url as string, m as Object) as string
    'print "Fetching HTML:JSON:http://192.168.1.150:3000/fetch-html?url=" ; url
    request = CreateObject("roUrlTransfer")
    request.SetUrl("http://192.168.1.150:3000/fetch-html?url=" + url)

    ' Fetch content as a string
    htmlContent = request.GetToString()

    if htmlContent = invalid or Len(htmlContent) = 0 then
    ...
    'print "HTML content length: "; Len(htmlContent)
    return htmlContent
end function
...
'-------------------------------------------
' Categorize HTML tags for further processing
'-------------------------------------------
function categorizeHTMLTag(tag as string, value as Object, indent as string)
    tagCategories = getTagCategories()

    if isInList(tag, tagCategories["containerTags"]) then
      '  handleContainerTag(tag, value, indent)
    else if isInList(tag, tagCategories["textTags"]) then
      '  handleTextTag(tag, value, indent)
    else if isInList(tag, tagCategories["mediaTags"]) then
      '  handleMediaTag(tag, value, indent)
    else if isInList(tag, tagCategories["linkTags"]) then
       ' handleLinkTag(tag, value, indent)
    else if isInList(tag, tagCategories["listTags"]) then
       ' handleListTag(tag, value, indent)
    else if isInList(tag, tagCategories["tableTags"]) then
       ' handleTableTag(tag, value, indent)
    else if isInList(tag, tagCategories["formTags"]) then
       ' handleFormTag(tag, value, indent)
    else if isInList(tag, tagCategories["headingTags"]) then
      '  handleHeadingTag(tag, value, indent)
    else if isInList(tag, tagCategories["headTags"]) then
      '  handleHeadTag(tag, value, indent)
    else
       ' 'print indent + "Uncategorized HTML Tag: " + tag
    end if
end function


' Function to combine all string entries from tagCategories and globalAttributes into a single list
function gethtmlKeys() as Object
combinedList = []
...
attributeTree = createAttributeTree()

' Combine tag categories
for each key in tagCategories.keys()
    tagList = tagCategories[key]
    for each tag in tagList
        combinedList.Push(tag)
    end for
end for

' Combine global attributes
for each key in attributeTree.keys()
    if key = "tagAttributes"
        tagAttributes = attributeTree[key]
        for each tagKey in tagAttributes.keys()
            tagList = tagAttributes[tagKey]
            for each tag in tagList
                combinedList.Push(tag)
            end for
        end for
